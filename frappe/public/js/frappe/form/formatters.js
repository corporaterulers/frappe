...
frappe.form.formatters = {
	...
	Float: function (value, docfield, options, doc) {
		...
		if (docfield.options && docfield.options.trim()) {
			...
			var formatted = format_number(value, null, precision);
			if (!(options || {}).always_show_decimals) {
				var decimal_separator = formatted.indexOf(".") !== -1 ? "." : formatted.indexOf(",") !== -1 ? "," : null;
				if (decimal_separator) {
					var trailing_zeros = new RegExp("(\\" + decimal_separator + "\\d*?[1-9])0+$");
					var zero_decimal = new RegExp("\\" + decimal_separator + "0+$");
					formatted = formatted.replace(trailing_zeros, "$1");
					formatted = formatted.replace(zero_decimal, "");
				}
			}
