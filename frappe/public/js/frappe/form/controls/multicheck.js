frappe.ui.form.ControlMultiCheck = class ControlMultiCheck extends frappe.ui.form.Control {
	...
	select_all(deselect = false) {
		$(this.wrapper).find(`:checkbox`).each((_, el) => {
			const $cb = $(el);
			const currently_checked = $cb.is(":checked");
			const should_be_checked = !deselect;
			if (currently_checked !== should_be_checked) {
				$cb.prop("checked", should_be_checked).trigger("change");
			}
		});
		$(this.wrapper).find(".unit-checkbox").toggleClass("checked-module", !deselect);
	}
	...
	select_options(selected_options) {
		this.options
			...
			.forEach((value) => {
				...
				if ($checkbox) {
					const is_checked = selected_options.includes(value);
					$checkbox.checked = is_checked;
					$($checkbox).closest(".unit-checkbox").toggleClass("checked-module", is_checked);
				}
			});
	}
